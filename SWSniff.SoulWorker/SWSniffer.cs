...
namespace SWSniff.SoulWorker
{
    public class SWSniffer : SnifferBase
    {
        ...
        public void Inject(PacketType t, ICanSerialize p, uint sockId)
        {
        ...
        public void Inject(PacketType t, byte[] dataPacket, uint sockId)
        {
        ...
        protected override void HandlePacket(uint socketId, byte[] msg, bool outgoing)
        {
            var data = msg;
            ...
            while (packetStart < data.Length)
            {
                ...
                PacketAction?.Invoke(this, new SnifferEventArgs(p, outgoing, socketId));
